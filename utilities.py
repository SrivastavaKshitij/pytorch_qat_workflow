...
def map_ckpt_names(model_state_dict):
    """
    takes in a model state dict and maps the keys to the new names
    """
...
def map_ckpt_names_inplace(model_state_dict):
    for key in list(model_state.keys()):
        if len(key.split('.')) == 4:
            if '.0.' in key:
                model_state[key.replace('.0.','.')]=model_state[key]
                del model_state[key]
            elif '.1.' in key:
                model_state[key.replace('.1.','.bn.')]=model_state[key]
                del model_state[key]
